            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <form method="GET" action="{{ route('konseling.index') }}" class="flex flex-wrap gap-2 items-end">
                            <div>
                        ...
                        @if (auth()->user()->role === 'BK')
                            <a href="{{ route('konseling.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm">+ Tambah Konseling</a>
                        @endif
                    </div>
                        ...
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr class="bg-gray-100 text-left">
                                    ...
                                    <th class="px-3 py-2">Jumlah Sesi</th>
                            ...
                            <tbody>
                                @forelse ($konseling as $item)
                                    <tr class="border-b">
                                        <td class="px-3 py-2">{{ $loop->iteration + ($konseling->currentPage() - 1) * $konseling->perPage() }}</td>
                                        <td class="px-3 py-2">{{ $item->tanggal?->format('d-m-Y') }}</td>
                                        <td class="px-3 py-2">{{ \Illuminate\Support\Str::limit($item->permasalahan, 60) }}</td>
                                        <td class="px-3 py-2">{{ $item->sesiKonseling->count() }}</td>
                                        <td class="px-3 py-2">
                                            <a href="{{ route('konseling.show', $item) }}" class="text-indigo-600 hover:underline">Detail</a>
                                        </td>
                                    </tr>
                                @empty
                    ...
                    <div class="mt-4">{{ $konseling->links() }}</div>
                </div>
            </div>
        </div>
    </div>
