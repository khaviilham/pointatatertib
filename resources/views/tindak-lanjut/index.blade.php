            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <form method="GET" action="{{ route('tindak-lanjut.index') }}" class="flex flex-wrap gap-2 items-end">
                            <div>
                        ...
                        @if (auth()->user()->role === 'BK')
                            <a href="{{ route('tindak-lanjut.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm">+ Tambah Tindak Lanjut</a>
                        @endif
                    </div>
                        ...
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr class="bg-gray-100 text-left">
                                    ...
                                    <th class="px-3 py-2">Jenis Tindakan</th>
                            ...
                            <tbody>
                                @forelse ($tindakLanjut as $item)
                                    <tr class="border-b">
                                        <td class="px-3 py-2">{{ $loop->iteration + ($tindakLanjut->currentPage() - 1) * $tindakLanjut->perPage() }}</td>
                                        <td class="px-3 py-2">{{ $item->tanggal?->format('d-m-Y') }}</td>
                                        <td class="px-3 py-2">
                                            @if ($item->pelanggaran)
                                                <a href="{{ route('pelanggaran.show', $item->pelanggaran) }}" class="text-indigo-600 hover:underline">#{{ $item->pelanggaran->id_pelanggaran }}</a>
                                            @else
                                                -
                                            @endif
                                        </td>
                                        <td class="px-3 py-2">{{ $item->pelanggaran->siswa->nama ?? '-' }}</td>
                                        <td class="px-3 py-2">{{ $item->jenis_tindakan }}</td>
                                        <td class="px-3 py-2">{{ $item->user->nama ?? '-' }}</td>
                                        <td class="px-3 py-2">
                                            <a href="{{ route('tindak-lanjut.show', $item) }}" class="text-indigo-600 hover:underline">Detail</a>
                                        </td>
                                    </tr>
                                @empty
                    ...
                    <div class="mt-4">{{ $tindakLanjut->links() }}</div>
                </div>
            </div>
        </div>
    </div>
